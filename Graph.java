...
public class Graph {
    ...
    public int findRoot() {
        ...
        int[] inDegree = new int[numVertices]; // counter for in-degrees in vertices

        // calculate in-degrees in vetex
        ...
        int rootIndex = -1; // set rootIndex to -1 if none are found

        // check if in-degrees in vertex equal 0
        for (int i = 0; i < numVertices; i++) {
            if (inDegree[i] == 0) {
                if (rootIndex == -1) {
                    rootIndex = i; // set to first in-degree candidate
                } else {
                    // if more than one vertex has in-degree == 0
                    return -1;
                }
            }
        }
